				...
				actions : {
										...
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("DocumentNumber") === oObject.DocumentNumber;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].DocumentNumber;
										} else {
											sCurrentId = aItemsNotInTheList[0].DocumentNumber;
										}
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							title: oBindingContext.getProperty("CreationDate")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "PurchaseOrdersSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'PurchaseOrdersSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("CreationDate");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all PurchaseOrdersSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "PurchaseOrdersSet (" + iExpectedLength + ")"}),
									success : function () {
